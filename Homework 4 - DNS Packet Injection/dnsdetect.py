...
def print_poisoning_info(packet, old_packet):
    ...
    print 'TXID', old_packet[DNS].id, 'Request', old_packet[
        DNS].qd.qname.rstrip('.')
...
def detect_dns_poison(packet):
    if packet.haslayer(IP) and packet.haslayer(UDP) and packet.haslayer(
            DNS) and packet.haslayer(DNSRR):
...
if __name__ == '__main__':
    ...
    if interface and tracefile:
        ...
        sniff(
            filter=expression, iface=interface, store=0, prn=detect_dns_poison)
    elif tracefile:
